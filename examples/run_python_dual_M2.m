...
datapath = 'Z:\projects\PSFLearning\exampledata\dualcolor_M2\'; % Z: t2ries 
datapath = strrep(strrep(datapath,'\',filesep),'/',filesep);

data_keyword = '3d';
subfolder = '3d'; % keyword for subfolder, leave it empty, if there is no subfolder
savename = 'psfmodel';
data_format = '.tif'; % '.mat', '.tif'
PSFtype = 'voxel'; % 'voxel', 'pupil','zernike','zernike_vector'
chtype = 'multi'; % 'single','multi','4pi'
...
pixelsize_x = 0.127; %um
pixelsize_y = 0.117; %um
pixelsize_z = 0.02;%um
bead_radius = 0.0;%um
...
iteration = 100;
FOV = struct('y_center',256,'x_center',128,'radius',80,'z_start',1,'z_end',0,'z_step',4); % if radius is zero, use the full FOV, 'z_start' is counting as 0,1,2..., 'z_end' is counting as 0,-1,-2...
...
plotall = 1; % if true, show the coordinates of selected beads, for multi or 4pi
...
optparam = struct('emission_wavelength',0.68,'RI',1.516,'RI_med',1.33,'RI_cov',1.516,'NA',1.43,'pupilsize',64,'n_max',9,'gauss_filter_sigma',2);
...
f.pixelsize_z = pixelsize_z*FOV.z_step;

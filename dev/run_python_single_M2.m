...
datapath = 'Z:\projects\PSFLearning\exampledata\40nm_M2\'; % Z: t2ries 
datapath = strrep(strrep(datapath,'\',filesep),'/',filesep);

data_keyword = 'bead';
subfolder = 'bead'; % keyword for subfolder, leave it empty, if there is no subfolder
savename = 'psfmodel';
...
pixelsize_x = 0.117; %um
pixelsize_y = 0.127; %um
pixelsize_z = 0.05;%um
bead_radius = 0.025;%um
...
iteration = 100;
FOV = struct('y_center',100,'x_center',100,'radius',30,'z_start',1,'z_end',0,'z_step',1); % if radius is zero, use the full FOV, 'z_start' is counting as 0,1,2..., 'z_end' is counting as 0,-1,-2...
